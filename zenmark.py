...
class Tokenizer:
	...
	def tokenize(self, segment):
		...
		if re.findall(r'(HS)([1-6])(.*?)$', segment):
			for t in re.findall(r'(HS)([1-6])(.*?)$', segment):
				...
				self.tag('tspan', t[2].strip(), fontSize = self.headingSize(t[1]), x = self.paperMarginLeft, dy = self.fontSpacing), 
				segment)
		...
		if re.findall(r'(HU)([1-6])(.*?)$', segment):
			for t in re.findall(r'(HU)([1-6])(.*?)$', segment):
				...
				self.tag('tspan', t[2].strip(), fontSize = self.headingSize(t[1]), x = self.paperMarginLeft, dy = self.fontSpacing, textDecoration = "underline"),
				segment)
		...
		if re.findall(r'(TS)([1-6])(.*?)$', segment):
			for t in re.findall(r'(TS)([1-6])(.*?)$', segment):
				...
				self.tag('tspan', t[2].strip(), fontSize = self.headingSize(t[1]), x = "50%", dy = self.fontSpacing, textAnchor = "middle"), 
				segment)
		...
		if re.findall(r'(TU)([1-6])(.*?)$', segment):
			for t in re.findall(r'(TU)([1-6])(.*?)$', segment):
				...
				self.tag('tspan', t[2].strip(), fontSize = self.headingSize(t[1]), x = "50%", dy = self.fontSpacing, textAnchor = "middle", textDecoration = "underline"), 
				segment)
		...
		if re.findall(r'(IN)([1-4])(.*?)$', segment):
			for t in re.findall(r'(IN)([1-4])(.*?)$', segment):
				if t[1] == "2":
					...
					self.tag('tspan', t[2].strip(), fontSize = 18, x = "50%", dy = self.fontSpacing, textAnchor = "middle", textDecoration = "underline", fontStyle = "italic"), 
					segment)
				if t[1] == "3":
					segment = re.sub(r'(IN)([1-4])(.*?)$',
					self.tag('tspan', t[2].strip(), fontSize = 14, x = "50%", dy = self.fontSpacing, textAnchor = "middle", fontStyle = "italic"), 
					segment)
				else:
					segment = re.sub(r'(IN)([1-4])(.*?)$',
					self.tag('tspan', t[2].strip(), fontSize = self.headingSize(t[1]), x = "50%", dy = self.fontSpacing, textAnchor = "middle"), 
	...
	def tag(self, name, *content, 
		...
		**attrs):
		...
		if textAnchor is not None:
			attrs['text-anchor'] = textAnchor
		
		if textDecoration is not None:
			attrs['text-decoration'] = textDecoration

		if space is not None:
			attrs['xml:space'] = space
	...
	def wordWrap(self, text, size, tab = False):
		#wrapPoint = ((self.paperBodyWidth - len(text)) / 2) - ((int(size) + 4) * 2)
		...
		for segment in wrappedText:
			...
			self.tokens.append(self.tag('tspan', segment, fontSize = size, x = self.paperMarginLeft, dy = spacing, space = "preserve"))
			count -= 1
